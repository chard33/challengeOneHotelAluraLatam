...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtFechaSalida.getJCalendar().getDayChooser().addPropertyChangeListener("day", new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent e) {
				if (txtFechaEntrada.getDate() != null) {
					...
					if(txtFechaSalida.getDate() != null){
						...
						final Double costoD = 333.33;
						
						Long diasD = ChronoUnit.DAYS.between(
								LocalDate.parse(((JTextField)txtFechaEntrada.getDateEditor().getUiComponent()).getText()),
								LocalDate.parse(((JTextField)txtFechaSalida.getDateEditor().getUiComponent()).getText()));
						
						txtValor.setText(new DecimalFormat("0.00").format((diasD+1)*costoD));
					}
				}else{
					txtFechaSalida.setMinSelectableDate(new Date(9999,12,1));
		...
		txtFormaPago.setModel(new DefaultComboBoxModel(new String[] {
				"Seleccione una opcion", "Tarjeta de Crédito", "Tarjeta de Débito",
				"Dinero en efectivo"}));
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null && txtFormaPago.getSelectedIndex() != 0) {
					
					controladorRegH rH = new controladorRegH();
					...
					if(JOptionPane.showConfirmDialog(null,
													 "Fecha de entrada: " + new SimpleDateFormat("yyyy-MM-dd").format(r.getFechaentrada()) + "\n" +
															 "Fecha de salida: " + new SimpleDateFormat("yyyy-MM-dd").format(r.getFechasalida()) + "\n" +
															 "Valor de reserva: " + r.getTotal() + "\n" +
															 "Forma de pago: " + r.getFormapago(),
													 "Datos de reserva correctos?",
													 JOptionPane.YES_NO_OPTION,
													 JOptionPane.WARNING_MESSAGE) == 0){
						Long nReserva = rH.guardarH(r);
						
						RegistroHuesped registro = new RegistroHuesped();
						
						registro.obtenerNr(nReserva);
